...
# ------------------------------------------------ artifacts, release tarballs

# Releases are built on the operator's machine from a clean commit and fetched
# by the instance at install time (scripts/deploy.py, deploy/install.sh). The
# instance needs no git credentials and no build toolchain, and what runs is
# exactly the tarball that was uploaded.
resource "aws_s3_bucket" "artifacts" {
  bucket = "atom-artifacts-${data.aws_caller_identity.current.account_id}-${var.env}"
  tags   = { Name = "atom-artifacts" }
}
