...
| V2-14 | **Correlation-matched harvest proxies** | v1 treats each universe as a single bucket and sets gains off against any loss inside it, with no proxy substitution | This is the entire `SECTOR-BUCKETS.md` / `BUCKET-MANAGEMENT.md` apparatus: three-tier taxonomy, Pearson-on-log-returns correlation, tier-1 exact-index and tier-2 group matching, the 0.85 floor, and the synthetic cost-basis carry-over (D-019). All researched and specified — parked, not discarded. **Deferring it means a harvest sale exits the position rather than rotating into an equivalent, so sector exposure is not preserved.** That is the trade-off v1 accepts |
| V2-13 | **Proactive tax-action proposals** | v1 stays descriptive outside harvesting | Beyond the position-specific LTCG warning (D-118), ATOM could surface planning nudges: unused ₹1.25 lakh exemption with N months of the FY left, a loss worth realising before 31 March, a set-off pool about to expire from the 8-year window. Deliberately excluded from v1 — it shades from reporting into advice |
