...
## Round 27 — 2026-09-24 · Broker research, round 2 (vendor documentation read directly)

Source for everything below: [`docs/03-brokers/API-REFERENCE-VERIFIED.md`](../03-brokers/API-REFERENCE-VERIFIED.md),
fetched 2026-09-24. Every claim names the vendor page it came from.

**D-173 — The static-IP design is validated by regulation, not merely permitted.**
SEBI's *Safer participation of retail investors in Algorithmic trading* circular and NSE
circular **NSE/INVG/67858** made a **registered static IP mandatory for API trading from
1 April 2026**. Upstox, Dhan and Shoonya all enforce it; Upstox rejects with `UDAPI1154`,
Shoonya blocks the OAuth flow itself.

D-009…D-011 designed a per-investor static egress IP from the brief's requirements, before this
was compulsory. It now is. Nothing changes in the design — but two constraints are added:

- **Dhan locks a registered IP for 7 days** (`GET /v2/ip/getIP` returns `modifyDatePrimary`).
  An instance rebuild that picks up a fresh address is a **week-long outage on Dhan**, not a
  restartable error. The Elastic IP is therefore **mandatory**, and must be allocated *before*
  the address is registered with any broker.
- **Dhan whitelists writes only** — reads work from any address. So the D-170 token probe
  (which reads holdings) does **not** exercise the proxy path there. *A token can probe green
  and the first order still fail on IP.* **Egress-IP verification becomes a separate pre-flight
  gate from token validity**, and `verify_egress_ip.py` runs on **every run**, not only at
  onboarding.

**D-174 — `MARKET` is removed from the live adapter surface.**
Market orders are **no longer permitted via API** (live 1 April 2026); Market Price Protection
is on by default. Upstox rejects with `UDAPI1158`. ATOM already places limit orders only
(D-057, Q-027), so nothing breaks — but the market-order path is not a fallback ATOM may keep in
reserve, and carrying a `MARKET` order type in the contract would imply an option that does not
exist. Dry-run may still simulate one for comparison; the live path may not emit one.

**D-175 — `client_ref`: one ATOM-generated order identifier, mapped per broker.**
Three brokers give ATOM a native way to stamp and recognise its own orders, and Groww gives a
true idempotency key:

| Broker | Field | Constraint | Idempotent |
|---|---|---|---|
| Groww | `order_reference_id` / `reference_id` | **required**, 8–20 alphanum, ≤ 2 hyphens | ✅ duplicate → `GA007`; and lookup **by ATOM's own id** |
| Dhan | `correlationId` | ≤ 30 chars | not stated |
| Zerodha | `tag` | alphanum, **≤ 20 chars** | `guid` described as a dedup request id |
| Upstox | `tag` | — | — |
| Shoonya | `remarks` | — | — |

The canonical model carries **one `client_ref`, generated by ATOM, ≤ 20 characters** so a single
format satisfies every broker, mapped by each adapter to the field above. On Groww a timeout
during placement is fully recoverable — retry with the same reference and either place the order
or learn it already exists. Elsewhere the adapter falls back to "query the book by tag before
retrying", which is a race, not a guarantee, and must be documented as such.

**`GA007` is handled as *success, already placed*, never as an error** — otherwise ATOM's retry
logic reports a false failure on a perfectly good order.

**D-176 — On Zerodha, ATOM's database is the only link to its own GTTs.**
An *active* Zerodha trigger returns `"meta": {}` or `null`; there is no tag field on a GTT at
all. The `app_id` appears only inside `orders[].result.meta` **after** the trigger fires.

So D-064 ("cancel only ATOM's own GTTs") **cannot be satisfied from broker data on Zerodha.**
The cancel-all-first step works entirely from ATOM's stored `trigger_id` mapping, and a GTT the
investor placed in Kite Web is indistinguishable from ATOM's until it fires.

Consequence: **the broker order ID is persisted before `place_gtt` returns, not after.** If that
mapping is lost, ATOM can neither cancel its own GTTs nor safely leave the investor's alone —
and on Zerodha there is no recovery path from the broker side. This is the strictest durability
requirement in the order layer, and it is driven by one broker's omission.

**D-177 — Three token patterns, not five screens.**
Round 1 concluded the console needs a per-broker token screen. Verified flows show it needs
**three**:

| Pattern | Brokers | Morning action |
|---|---|---|
| **A — Headless** (TOTP / checksum) | Dhan, Groww¹, Shoonya² | None, or one approval click |
| **B — Hosted redirect** | Upstox, Zerodha, Dhan (key+secret), Shoonya | Click "Login", authenticate, token lands at the redirect |
| **C — Paste-in** | **All five** | Copy from the broker, paste into ATOM |

¹ Groww's checksum/TOTP call is headless but still needs a **daily approval click** in Groww's
console. ² Shoonya's code step still appears interactive.

**Pattern C is built for all five first, as the guaranteed fallback** — it is the only path a
vendor cannot break by changing redirect handling. A and B are layered on per broker afterwards.
Upstox's **semi-automated / notifier-URL** flow (operator approves on mobile, token is delivered
to a URL ATOM owns) is the closest any broker comes to what the brief asked for and is the
preferred Upstox path.

Two scheduling constraints fall out: **Zerodha and Groww tokens both expire at 6:00 AM**, so
token generation and the run must both sit **after 06:00 IST**. And **Zerodha alone lets ATOM
actively destroy the token** (`DELETE /session/token`) — D-170's `CLEARED` step calls it there
and merely forgets the token elsewhere.

**D-178 — The token probe prefers a profile endpoint where one exists.**
D-170 specifies probing holdings. Dhan's `GET /v2/profile` returns `tokenValidity`,
`activeSegment`, `ddpi`, `mtf`, `dataPlan` and `dataValidity` — it is cheaper, is documented as
"a great test API", and tells ATOM **why** an account is unusable rather than only that it is.
The adapter contract gains a `get_profile()` method; the probe uses it where implemented and
falls back to holdings where not. D-170's principle is unchanged — validity is **tested, never
computed from a documented lifetime**.

Dhan's `givenPowerOfAttorney` (DDPI) flag is captured at onboarding, since it determines whether
sells need per-trade authorisation.

**D-179 — The charges-contrast view has a ground truth on Dhan and a model elsewhere.**
`GET /v2/trades/{from}/{to}/{page}` returns, per trade: `sebiTax`, `stt`, `brokerageCharges`,
`serviceTax`, `exchangeTransactionCharges`, `stampDuty`, plus `isin` and `exchangeTradeId`.

So D-024/D-105's estimated-vs-actual contrast is **real per fill and per component on Dhan**.
Zerodha, Groww and Shoonya expose no per-trade charge breakdown in their documentation (Q-273),
so the view degrades to estimate-only there — and **that degradation is shown in the UI, not
silent.** A user must never be left thinking they are looking at reported charges when they are
looking at ATOM's model.

Dhan's `GET /v2/ledger` adds `runbal` (running balance), which becomes the reconciliation anchor
for the cost-of-capital model: computed `principal_outstanding` is checked against the broker's
own running balance, not only against ATOM's internal ledger.

**D-180 — Unknown order statuses are in-flight, never terminal.**
Zerodha's status field is not a closed enum. Beyond `OPEN` / `COMPLETE` / `CANCELLED` /
`REJECTED` it publishes `PUT ORDER REQ RECEIVED`, `VALIDATION PENDING`, `OPEN PENDING`,
`MODIFY VALIDATION PENDING`, `TRIGGER PENDING`, `CANCEL PENDING`, `AMO REQ RECEIVED` — and warns
"there may be other values as well".

Every adapter's status mapping therefore has a **default branch that maps an unrecognised status
to in-flight**. Mapping an unknown status to "failed" would make ATOM re-place an order that is
about to fill — the one order-layer bug that can lose real money silently.

---

### Questions raised by round 2

| ID | Question | Blocks |
|---|---|---|
| **Q-268** 🔴 | Does ATOM need a **broker-empanelled Algo ID** at 2 OPS? **Upstox says registration is needed only above 10 OPS; Shoonya says every API order needs an Algo ID or is rejected at the exchange.** Must be answered by each broker's compliance desk | **Going live on any broker** |
| **Q-269** | Shoonya OAuth access-token lifetime — the `manual-login-oauth` page returns 403 to automated fetch | Shoonya adapter |
| **Q-270** 🔴 | **EDIS authorization** must be completed per Upstox account before any sell GTT will work. One-time, manual, done from Upstox Web/iOS/Android. Who does it and when? | Upstox sell side |
| **Q-271** | Shoonya GTT endpoint and alert-type enum — still not published (restates Q-237 against the rebuilt docs) | Phase E |
| **Q-272** | `HOLDINGS-ATTRIBUTION.md` says `broker_quantity` without defining it. Groww returns `quantity`, `t1_quantity`, `demat_free_quantity`, `pledge_quantity` and more. Should the identity be pinned to **`demat_free_quantity`** (the sellable figure)? | Attribution correctness |
| **Q-273** | Do Zerodha, Groww or Shoonya expose per-trade charges via API? None found | Charges contrast fidelity |
| **Q-274** | Static-IP whitelisting procedure for Zerodha and Groww — not in their developer docs | Onboarding |
| **Q-275** | Cost of Dhan's Data API plan and Groww's Trading API subscription, per account | Running cost model |
| **Q-276** | Build the Dhan `voucherdesc` → ATOM cash-event mapping from a real ledger pull (`"PAYBNK"` = withdrawal is the only one seen) | Cost-of-capital classifier |
| **Q-277** | If an Algo ID is required, does changing a configurable threshold count as a strategy change needing re-registration? Shoonya: "keep static strategies static" | Config model |

### What round 2 did **not** change

Worth stating explicitly, because most of the design survived contact with the vendor docs
unchanged: the universe model, the deviation metric, the sell logic, the tax engine, the
